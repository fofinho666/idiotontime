...
const { goTo, findByIdAndFillIn, click, waitForNetwork, createEntry } = require("./puppeteer-actions")
...
const getDatesToClockIn = (userSettings) => {
  ...
  let endDate = parse(userSettings.end_date || defaltEndDateString, "dd-MM-yyyy", todayDate)
  if (endDate.getTime() < startDate.getTime()) endDate=startDate

  const holidays = new Holidays(userSettings.country)
